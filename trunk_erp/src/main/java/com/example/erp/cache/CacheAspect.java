...
public class CacheAspect {
    ...
    * */
     ...
     *
     * */
    private static final String POINTCUT_DEPT_UPDATE = "execution(* com.example.erp.service.impl.DeptServiceImpl.updateById(..))";
    private static final String POINTCUT_DEPT_ADD = "execution(* com.example.erp.service.impl.DeptServiceImpl.save(..))";
    private static final String POINTCUT_DEPT_GET = "execution(* com.example.erp.service.impl.DeptServiceImpl.getById(..))";
    private static final String POINTCUT_DEPT_DELETE = "execution(* com.example.erp.service.impl.DeptServiceImpl.removeById(..))";
    ...
    public Object cacheDeptGet(ProceedingJoinPoint joinPoint)throws Throwable{
        ...
        if (res1!=null){
                ...
                log.info("未从缓存里面找到部门对象,去数据库中查询并放入缓存"+CACHE_DEPT_PROFIX+res2.getId());
                return res2;
        }
    }
     ...
     *
     * */
    private static final String POINTCUT_USER_UPDATE = "execution(* com.example.erp.service.impl.UserServiceImpl.updateById(..))";
    private static final String POINTCUT_USER_ADD = "execution(* com.example.erp.service.impl.UserServiceImpl.save(..))";
    private static final String POINTCUT_USER_GET = "execution(* com.example.erp.service.impl.UserServiceImpl.getById(..))";
    private static final String POINTCUT_USER_DELETE = "execution(* com.example.erp.service.impl.UserServiceImpl.removeById(..))";
    ...
    public Object cacheUserGet(ProceedingJoinPoint joinPoint)throws Throwable{
        ...
        if (res1!=null){
            log.info("从缓存里面找到用户对象"+CACHE_USER_PROFIX+object);
            return res1;
        }else {
            User res2 = (User) joinPoint.proceed();
            CACHE_CONTAINER.put(CACHE_USER_PROFIX+res2.getId(),res2);
            log.info("未从缓存里面找到用户对象,去数据库中查询并放入缓存"+CACHE_USER_PROFIX+res2.getId());
            return res2;
        }
    }
    ...
    public Object cacheUserAdd(ProceedingJoinPoint joinPoint)throws Throwable{
        ...
        if (res){
            ...
            log.info("添加成功并且加入到缓存"+CACHE_USER_PROFIX+object.getId());
        }
        return res;
    }
